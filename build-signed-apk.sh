#!/usr/bin/env bash
#
# build-signed-apk.sh — Compila un APK release FIRMADO con tu propio keystore
...
# - Compila :app:assembleFossRelease (firmado).
...
info "Compilando :app:assembleFossRelease (firmado)"
./gradlew :app:assembleFossRelease

# Cuando hay keystore.properties, Gradle produce "app-foss-release.apk" firmado.
APK="$(ls -t "$REPO_DIR"/app/build/outputs/apk/foss/release/app-foss-release.apk 2>/dev/null | head -1)"
if [ -z "$APK" ]; then
    APK="$(ls -t "$REPO_DIR"/app/build/outputs/apk/foss/release/app-foss-release-unsigned.apk 2>/dev/null | head -1)"
fi
if [ -z "$APK" ] || [ ! -f "$APK" ]; then
...
if [ -n "$WUSER" ] && [ -d "/mnt/c/Users/$WUSER" ]; then
    OUT_NAME="Ausgaben-es-release.apk"
...
=============================================================
  Completo. APK release FIRMADO listo.
=============================================================
  APK:            $APK
